...
!   1.1.9   08/15/16    : removed allRhopij and allRhonij
...
!            tempBigJ = int(2.*intJ)-1
        ...
        call Projection_with_Parity(dble(Jmax),dbleInt(bigJmax),psdf,nsdf,tolerance, & !populates N_ijk, H_ijk
                ParityTest,alpha_i,beta_j,gamma_k,N_ijk,H_ijk,PN_ijk,PH_ijk,allOvlpp,allOvlpn,.true.)
...
!                tempDeltaJpJ = deltaJpJ
    ...
    if (.not.ParityTest) posdefP(2) = .false. !necessary(?)
    call Projection_with_Parity(dble(Jmax),dbleInt(bigJmax),psdf,nsdf,tolerance, & !populates N_ijk, H_ijk
            ParityTest,alpha_i,beta_j,gamma_k,N_ijk,H_ijk,PN_ijk,PH_ijk,allOvlpp,allOvlpn,.false.)
...
!.... ADDED! DIAGONALIZE N_J_MK TO GET OUT # of
        deallocate(TSDNmat)
    end do
