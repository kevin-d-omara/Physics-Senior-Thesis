...
interface
   ...
   SUBROUTINE J_WriteResults(npts,tol,np,nf,jvals,pevals,nlist,jlist,problist,parityflag,jmin,jmax,isOdd)  ! INTERFACE
   USE errortests
   IMPLICIT NONE
   real(kind=8), intent(in) :: jmin, jmax
...
!	WRITE(*,1006)
	...
	call J_WriteResults(npts,tol,npmax,nftotal,jall,pallPair,nlist,jlist,problist,pairtest,jmin,jmax,isOdd)
!	IF (pairTest) THEN
...
SUBROUTINE J_WriteResults(npts,tol,np,nf,jvals,pevals,nlist,jlist,problist,parityflag,jmin,jmax,isOdd)
...
! write npts & fractionJ to relevant files for graphing purposes
